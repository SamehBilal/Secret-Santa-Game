...
<body>
    ...
    <a href="{{ route('wheel') }}" title="Soon"
        id="play">{{ auth()->user()->user_id != null ? 'Result' : 'Play' }}</a>
    <main class="wrapper">
        ...
        <form method="POST" action="{{ route('dashboard.profile', $user->id) }}" enctype="multipart/form-data">
            ...
            <div class="dropdown">
                ...
                <input type="text" name="fullname" id="fullname" value="{{ $user->full_name }}" autocomplete="off">
                <label for="monster-select">Avatar:</label>
                <select name="avatar" id="monster-select">
                    @foreach ($avatars as $avatar)
                        <option value="{{ $avatar }}" {{ $user->avatar == $avatar ? 'selected' : '' }}>
                            {{ $avatar }}</option>
                    @endforeach
                </select>
                <label for="nickname">Nickname:</label>
                <input type="text" name="nickname" id="nickname" value="{{ $user->nickname }}" autocomplete="off">
                @if ($user->password == '$2y$10$5jBp6xj9JMSK2UIfhVCx1Ok18OgKz/p/sbRXq.Zht8ZFVCCYENeF2')
                ...
                <select name="category" id="category">
                    <option value="Food and Drinks" {{ $user->category == 'Food and Drinks' ? 'selected' : '' }}>Food
                        and
                        Drinks</option>
                    <option value="Electronics" {{ $user->category == 'Electronics' ? 'selected' : '' }}>Electronics
                    </option>
                    <option value="Books" {{ $user->category == 'Books' ? 'selected' : '' }}>Books</option>
                    <option value="Home and Kitchen" {{ $user->category == 'Home and Kitchen' ? 'selected' : '' }}>Home
                        and Kitchen</option>
                    <option value="Toys and Games" {{ $user->category == 'Toys and Games' ? 'selected' : '' }}>Toys and
                        Games</option>
                    <option value="Beauty and Personal Care"
                        {{ $user->category == 'Beauty and Personal Care' ? 'selected' : '' }}>Beauty and Personal Care
                    </option>
                    <option value="Clothing and Accessories"
                        {{ $user->category == 'Clothing and Accessories' ? 'selected' : '' }}>Clothing and Accessories
                    </option>
                    <option value="Sports and Outdoors"
                        {{ $user->category == 'Sports and Outdoors' ? 'selected' : '' }}>
                        Sports and Outdoors</option>
                    <option value="Gift Cards" {{ $user->category == 'Gift Cards' ? 'selected' : '' }}>Gift Cards
                    </option>
                    <option value="Subscription Services"
                        {{ $user->category == 'Subscription Services' ? 'selected' : '' }}>Subscription Services
                    </option>
                </select>
